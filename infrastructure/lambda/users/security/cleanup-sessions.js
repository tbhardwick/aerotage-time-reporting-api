"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.handler = void 0;
var client_dynamodb_1 = require("@aws-sdk/client-dynamodb");
var lib_dynamodb_1 = require("@aws-sdk/lib-dynamodb");
var client = new client_dynamodb_1.DynamoDBClient({});
var docClient = lib_dynamodb_1.DynamoDBDocumentClient.from(client);
var handler = function (event, context) { return __awaiter(void 0, void 0, void 0, function () {
    var result, sessions, now, sessionsToDelete, _i, sessions_1, session, shouldDelete, deletedCount, error_1;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                console.log('Starting session cleanup job:', JSON.stringify(event, null, 2));
                result = {
                    totalSessions: 0,
                    expiredSessions: 0,
                    inactiveSessions: 0,
                    orphanedSessions: 0,
                    deletedSessions: 0,
                    errors: 0,
                };
                _a.label = 1;
            case 1:
                _a.trys.push([1, 5, , 6]);
                return [4 /*yield*/, getAllSessions()];
            case 2:
                sessions = _a.sent();
                result.totalSessions = sessions.length;
                console.log("Found ".concat(sessions.length, " total sessions to analyze"));
                if (sessions.length === 0) {
                    console.log('No sessions found, cleanup complete');
                    return [2 /*return*/, result];
                }
                now = new Date();
                sessionsToDelete = [];
                for (_i = 0, sessions_1 = sessions; _i < sessions_1.length; _i++) {
                    session = sessions_1[_i];
                    try {
                        shouldDelete = shouldDeleteSession(session, now);
                        if (shouldDelete.delete) {
                            sessionsToDelete.push(session.sessionId);
                            // Track cleanup reasons
                            if (shouldDelete.reason === 'expired') {
                                result.expiredSessions++;
                            }
                            else if (shouldDelete.reason === 'inactive') {
                                result.inactiveSessions++;
                            }
                            else if (shouldDelete.reason === 'orphaned') {
                                result.orphanedSessions++;
                            }
                        }
                    }
                    catch (error) {
                        console.error("Error analyzing session ".concat(session.sessionId, ":"), error);
                        result.errors++;
                    }
                }
                console.log("Sessions to delete: ".concat(sessionsToDelete.length));
                console.log("  Expired: ".concat(result.expiredSessions));
                console.log("  Inactive: ".concat(result.inactiveSessions));
                console.log("  Orphaned: ".concat(result.orphanedSessions));
                if (!(sessionsToDelete.length > 0)) return [3 /*break*/, 4];
                return [4 /*yield*/, deleteSessions(sessionsToDelete)];
            case 3:
                deletedCount = _a.sent();
                result.deletedSessions = deletedCount;
                _a.label = 4;
            case 4:
                console.log("Session cleanup completed successfully:");
                console.log("  Total sessions analyzed: ".concat(result.totalSessions));
                console.log("  Sessions deleted: ".concat(result.deletedSessions));
                console.log("  Errors: ".concat(result.errors));
                return [2 /*return*/, result];
            case 5:
                error_1 = _a.sent();
                console.error('Session cleanup job failed:', error_1);
                result.errors++;
                return [2 /*return*/, result];
            case 6: return [2 /*return*/];
        }
    });
}); };
exports.handler = handler;
/**
 * Get all sessions from the table
 */
function getAllSessions() {
    return __awaiter(this, void 0, void 0, function () {
        var sessions, lastEvaluatedKey, scanCommand, result, error_2;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    sessions = [];
                    lastEvaluatedKey = undefined;
                    _a.label = 1;
                case 1:
                    _a.trys.push([1, 3, , 4]);
                    scanCommand = new lib_dynamodb_1.ScanCommand({
                        TableName: process.env.USER_SESSIONS_TABLE,
                        ExclusiveStartKey: lastEvaluatedKey,
                        Limit: 100, // Process in batches
                    });
                    return [4 /*yield*/, docClient.send(scanCommand)];
                case 2:
                    result = _a.sent();
                    if (result.Items) {
                        sessions.push.apply(sessions, result.Items);
                    }
                    lastEvaluatedKey = result.LastEvaluatedKey;
                    return [3 /*break*/, 4];
                case 3:
                    error_2 = _a.sent();
                    console.error('Error scanning sessions:', error_2);
                    return [3 /*break*/, 5];
                case 4:
                    if (lastEvaluatedKey) return [3 /*break*/, 1];
                    _a.label = 5;
                case 5: return [2 /*return*/, sessions];
            }
        });
    });
}
/**
 * Determine if a session should be deleted
 */
function shouldDeleteSession(session, now) {
    // Check if session has expired based on expiresAt
    var expiresAt = new Date(session.expiresAt);
    if (expiresAt <= now) {
        return { delete: true, reason: 'expired' };
    }
    // Check if session is marked as inactive
    if (!session.isActive) {
        return { delete: true, reason: 'inactive' };
    }
    // Check if session has been inactive for too long based on session timeout
    var lastActivity = new Date(session.lastActivity || session.loginTime);
    var sessionTimeoutMinutes = session.sessionTimeout || 480; // Default 8 hours
    var timeoutMs = sessionTimeoutMinutes * 60 * 1000;
    if ((now.getTime() - lastActivity.getTime()) > timeoutMs) {
        return { delete: true, reason: 'expired' };
    }
    // Check for orphaned sessions (sessions older than 30 days regardless of activity)
    var thirtyDaysAgo = new Date(now.getTime() - (30 * 24 * 60 * 60 * 1000));
    var createdAt = new Date(session.createdAt || session.loginTime);
    if (createdAt <= thirtyDaysAgo) {
        return { delete: true, reason: 'orphaned' };
    }
    // Session is still valid
    return { delete: false };
}
/**
 * Delete sessions in batches
 */
function deleteSessions(sessionIds) {
    return __awaiter(this, void 0, void 0, function () {
        var deletedCount, batchSize, i, batch, deletePromises, error_3, _i, batch_1, sessionId, deleteCommand, individualError_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    deletedCount = 0;
                    batchSize = 25;
                    i = 0;
                    _a.label = 1;
                case 1:
                    if (!(i < sessionIds.length)) return [3 /*break*/, 14];
                    batch = sessionIds.slice(i, i + batchSize);
                    _a.label = 2;
                case 2:
                    _a.trys.push([2, 4, , 11]);
                    deletePromises = batch.map(function (sessionId) {
                        var deleteCommand = new lib_dynamodb_1.DeleteCommand({
                            TableName: process.env.USER_SESSIONS_TABLE,
                            Key: { sessionId: sessionId },
                        });
                        return docClient.send(deleteCommand);
                    });
                    return [4 /*yield*/, Promise.all(deletePromises)];
                case 3:
                    _a.sent();
                    deletedCount += batch.length;
                    console.log("Deleted batch of ".concat(batch.length, " sessions (").concat(deletedCount, "/").concat(sessionIds.length, " total)"));
                    return [3 /*break*/, 11];
                case 4:
                    error_3 = _a.sent();
                    console.error("Error deleting batch of sessions:", error_3);
                    _i = 0, batch_1 = batch;
                    _a.label = 5;
                case 5:
                    if (!(_i < batch_1.length)) return [3 /*break*/, 10];
                    sessionId = batch_1[_i];
                    _a.label = 6;
                case 6:
                    _a.trys.push([6, 8, , 9]);
                    deleteCommand = new lib_dynamodb_1.DeleteCommand({
                        TableName: process.env.USER_SESSIONS_TABLE,
                        Key: { sessionId: sessionId },
                    });
                    return [4 /*yield*/, docClient.send(deleteCommand)];
                case 7:
                    _a.sent();
                    deletedCount++;
                    return [3 /*break*/, 9];
                case 8:
                    individualError_1 = _a.sent();
                    console.error("Error deleting individual session ".concat(sessionId, ":"), individualError_1);
                    return [3 /*break*/, 9];
                case 9:
                    _i++;
                    return [3 /*break*/, 5];
                case 10: return [3 /*break*/, 11];
                case 11:
                    if (!(i + batchSize < sessionIds.length)) return [3 /*break*/, 13];
                    return [4 /*yield*/, new Promise(function (resolve) { return setTimeout(resolve, 100); })];
                case 12:
                    _a.sent();
                    _a.label = 13;
                case 13:
                    i += batchSize;
                    return [3 /*break*/, 1];
                case 14: return [2 /*return*/, deletedCount];
            }
        });
    });
}
