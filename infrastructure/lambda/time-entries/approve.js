...
var handler = function (event) { return __awaiter(void 0, void 0, void 0, function () {
    var authContext, userId, userRole, requestData, validationErrors, approvalErrors, _i, _a, timeEntryId, timeEntry, error_1, allowSelfApproval, result, statusCode, message, response, error_2;
    var _b, _c;
    return __generator(this, function (_d) {
        switch (_d.label) {
                ...
                if (userRole !== 'manager' && userRole !== 'admin') {
                    return [2 /*return*/, {
                            ...
                            body: JSON.stringify({
                                success: false,
                                error: {
                                    code: types_1.TimeEntryErrorCodes.INSUFFICIENT_APPROVAL_PERMISSIONS,
            ...
            case 3:
                timeEntry = _d.sent();
                if (!timeEntry) {
                    approvalErrors.push("Time entry ".concat(timeEntryId, " not found"));
                    return [3 /*break*/, 5];
                }
                // Check if time entry is in an approvable state
                if (timeEntry.status !== 'submitted') {
                    approvalErrors.push("Time entry ".concat(timeEntryId, " is not submitted for approval (status: ").concat(timeEntry.status, ")"));
                    return [3 /*break*/, 5];
                }
                ...
                if (timeEntry.userId === userId) {
                    if (userRole === 'employee') {
                        approvalErrors.push("Employees cannot approve their own time entry (".concat(timeEntryId, ")"));
                        return [3 /*break*/, 5];
                    }
                    // Managers and admins can approve their own entries
                    console.log("Self-approval allowed for ".concat(userRole, ": ").concat(timeEntryId));
                }
                return [3 /*break*/, 5];
            case 4:
                error_1 = _d.sent();
                approvalErrors.push("Error checking time entry ".concat(timeEntryId, ": ").concat(error_1.message));
                return [3 /*break*/, 5];
            case 5:
                _i++;
                return [3 /*break*/, 1];
            case 6:
                ...
                allowSelfApproval = userRole === 'manager' || userRole === 'admin';
                return [4 /*yield*/, timeEntryRepo.approveTimeEntries(requestData.timeEntryIds, userId, allowSelfApproval)];
            case 7:
                result = _d.sent();
                console.log("Approved ".concat(result.successful.length, " time entries, ").concat(result.failed.length, " failed"));
                statusCode = 200;
                message = 'All time entries approved successfully';
            ...
            case 8:
                error_2 = _d.sent();
                console.error('Error approving time entries:', error_2);
                return [2 /*return*/, {
                        statusCode: 500,
                        headers: {
                            'Content-Type': 'application/json',
